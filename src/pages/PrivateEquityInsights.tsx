...
const PrivateEquityInsights = () => {
  ...
  const backUrl = location.state?.backUrl || '/accounts/private-equity';
      ...
      try {
        ...
        const fullAccounts = await Promise.all(matchedAccountsBasic.map(async (a: { id: string }) => {
          ...
          try {
            ...
            // Thoroughly unwrap the response to get the actual insights object
        ...
        <div className="flex flex-col gap-4 pb-4 border-b border-slate-200/60">
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => navigate(`/private-equity/${id}`, { state: { backUrl } })}
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-400 mt-0.5 flex items-center gap-2">
                <span>{firm?.name || "Vanguard Tech Fund III"}</span>
                <span className="text-slate-300">•</span>
                <span>Last updated: {parsedInsights?.generated_at ? new Date(parsedInsights.generated_at).toLocaleString() : 'Just now'}</span>
              </p>
            </div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredAccountsTable.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={5} className="h-32 text-center text-xs font-bold text-slate-400">
                        ...
                        return (
                          ...
                          >
                            <TableCell className="pl-6 py-4">
                              ...
                              >
                                <span className="font-bold text-sm text-blue-600 hover:text-blue-700 hover:underline transition-colors block cursor-pointer">
                                  {acc.name}
                                </span>
                              </div>
                            </TableCell>
                              ...
                              {(() => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <span className={`text-xs font-bold ${isUnassigned ? 'text-slate-400 italic text-blue-600 hover:underline' : 'text-slate-700 text-blue-600 hover:underline'}`}>{displayName}</span>
                                  </div>
                                );
                              })()}
                            </TableCell>
