...
import {
  ...
  Menu, ChevronRight, ChevronDown, ShieldAlert, Bot, Users, Activity, Swords, Award, FileText, Map, List
} from 'lucide-react';
import {
...
import { Account as FinanceAccount, Project as FinanceProject } from '@/types/finance-database';
...
const FinancialAccountDetails = () => {
  ...
  const { accounts, fetchAccount, refreshAccounts } = useAccounts();
  const [salesAccount, setSalesAccount] = useState<AccountWithProjects | null>(null);
  ...
  useEffect(() => {
    if (data && accounts) {
      // 1. Try to find by ID
      let match = accounts.find((a: AccountWithProjects) => a.account_id === data.id);
      // 2. Try to find by name
      if (!match) {
        const searchName = (data.name || "").trim().toLowerCase();
        match = accounts.find((a: AccountWithProjects) => {
      ...
      if (match) {
        ...
        setSalesAccount({
          ...
          domain: '-',
          account_focus: undefined,
          engagement_age: '-',
          account_research_link: '',
          delivery_owner: '-',
          team_size: '-',
          overall_delivery_health: '-',
          current_rate_card_health: undefined,
          engagement_models: '-',
      ...
      <MainLayout>
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="text-center py-16">
            ...
            <Button onClick={() => navigate(backUrl)}>Back to Financials</Button>
          </div>
        </div>
      </MainLayout>
    );
  ...
  const account: AccountWithProjects = salesAccount || {
    account_id: '',
    account_name: '',
    projects: [],
    status: 'ACTIVE',
  ...
  const sortedProjects = [...projectsList].sort((a: FinanceProject, b: FinanceProject) => (b.total_revenue || 0) - (a.total_revenue || 0));

  const filteredProjects = sortedProjects.filter((p: FinanceProject) =>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
          <Card className="bg-white border border-slate-100 shadow-sm rounded-xl p-4 flex flex-col justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xl font-bold text-slate-900">{formatCurrency(data.current_revenue || data.total_revenue || 0)}</p>
            </div>
          ...
          <Card className="bg-white border border-slate-100 shadow-sm rounded-xl p-4 flex flex-col justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xl font-bold text-slate-900">{formatCurrency(data.target_revenue || 0)}</p>
            </div>
          ...
          <Card className="bg-white border border-slate-100 shadow-sm rounded-xl p-4 flex flex-col justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xl font-bold text-slate-900">{formatCurrency(data.forecast_revenue || 0)}</p>
            </div>
          ...
          <Card className="bg-white border border-slate-100 shadow-sm rounded-xl p-4 flex flex-col justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xl font-bold text-red-600">{formatCurrency(((data as { shortfall?: number }).shortfall ?? ((data.target_revenue || 0) - (data.current_revenue || data.total_revenue || 0) - (data.forecast_revenue || 0))))}</p>
            </div>
          ...
          <Card className="bg-white border border-slate-100 shadow-sm rounded-xl p-4 flex flex-col justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xl font-bold text-blue-600">{formatCurrency(data.ai_revenue || 0)}</p>
            </div>
          ...
          <Card className="bg-white border border-slate-100 shadow-sm rounded-xl p-4 flex flex-col justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xl font-bold text-blue-600">{(data.ai_penetration_pct || 0).toFixed(1)}%</p>
            </div>
          ...
          <Card className="bg-white border border-slate-100 shadow-sm rounded-xl p-4 flex flex-col justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xl font-bold text-slate-900">{data.active_project_count + data.inactive_project_count}</p>
            </div>
        ...
        <Tabs defaultValue="ai insights" className="w-full">
          ...
          <TabsContent value="sales & account details" className="mt-0">
            ...
            <Tabs defaultValue="ai recommendations" className="w-full">
              ...
              <TabsContent value="roadmaps" className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <TabsContent value="sales details" className="mt-0 animate-in fade-in slide-in-from-right-4 duration-500">
                <div className="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
                  ...
                  {openSections['sales_info'] && (
                    <div className="p-6 border-t border-slate-100 space-y-8 animate-in fade-in duration-300">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-5">
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Domain</span><p className="text-sm font-semibold text-slate-900">{account.domain || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Account Focus</span><p className="text-sm font-semibold text-slate-900">{account.account_focus || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Engagement Age</span><p className="text-sm font-semibold text-slate-900">{account.engagement_age || '-'}</p></div>
                          <div className="space-y-1 md:col-span-3">
                            ...
                            {account.account_research_link ? (
                              <a href={account.account_research_link} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline block text-sm font-semibold">View Market Report ↗</a>
                            ) : <span className="text-sm text-slate-400 font-medium block">No link provided.</span>}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4 pt-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-5">
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Delivery Owner</span><p className="text-sm font-semibold text-slate-900">{account.delivery_owner || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Team Size</span><p className="text-sm font-semibold text-slate-900">{account.team_size || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Active Projects</span><p className="text-sm font-semibold text-slate-900">{account.number_of_active_projects || '-'}</p></div>
                          <div className="space-y-1">
                            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Overall Health</span>
                            <div className="mt-0.5">
                              <Badge variant="outline" className={cn("border-none px-2 py-0.5 text-xs font-semibold",
                          ...
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Rate Card Health</span><p className="text-sm font-semibold text-slate-900">{account.current_rate_card_health || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Engagement Models</span><p className="text-sm font-semibold text-slate-900">{account.engagement_models || '-'}</p></div>
                          <div className="space-y-1 md:col-span-3"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Current Engagement Areas</span><p className="text-sm font-semibold text-slate-900">{account.current_engagement_areas || '-'}</p></div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4 pt-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-5">
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Company Revenue</span><p className="text-sm font-semibold text-slate-900">{account.company_revenue || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Last Year Business</span><p className="text-sm font-semibold text-slate-900">{account.last_year_business_done || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Target 2026 (Accounts)</span><p className="text-sm font-semibold text-slate-900">{account.target_projection_2026_accounts || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Target 2026 (Delivery)</span><p className="text-sm font-semibold text-slate-900">{account.target_projection_2026_delivery || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Current Pipeline</span><p className="text-sm font-semibold text-slate-900">{account.current_pipeline_value || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Attrition / Leakage Risk</span><p className="text-sm font-semibold text-slate-900">{account.revenue_attrition_possibility || '-'}</p></div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4 pt-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-5">
                          <div className="space-y-1">
                            ...
                            <div className="mt-0.5"><Badge variant={account.know_customer_value_chain ? 'default' : 'secondary'} className="bg-indigo-600">{account.know_customer_value_chain ? 'Yes' : 'No'}</Badge></div>
                          </div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Value Chain Fit</span><p className="text-sm font-semibold text-slate-900">{account.where_we_fit_in_value_chain || 'Not detailed.'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Roadmap Visibility (2026)</span><p className="text-sm font-semibold text-slate-900">{account.visibility_client_roadmap_2026 || 'No visibility.'}</p></div>
                          <div className="space-y-1">
                            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">30 Days Growth Action Plan Ready?</span>
                            <div className="mt-0.5"><Badge variant={account.growth_action_plan_30days_ready ? 'default' : 'secondary'} className="bg-indigo-600">{account.growth_action_plan_30days_ready ? 'Yes' : 'No'}</Badge></div>
                          </div>
                          <div className="space-y-1 md:col-span-2"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Cross-Sell Areas</span><p className="text-sm font-semibold text-slate-900">{account.identified_areas_cross_up_selling || 'None identified.'}</p></div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4 pt-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-5">
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Review Cadence</span><p className="text-sm font-semibold text-slate-900">{account.account_review_cadence_frequency || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Technical Audit</span><p className="text-sm font-semibold text-slate-900">{account.technical_audit_frequency || '-'}</p></div>
                          <div className="space-y-1">
                            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">QBR Happening?</span>
                            <div className="mt-0.5"><Badge variant={account.qbr_happening ? "default" : "secondary"}>{account.qbr_happening ? "Yes" : "No"}</Badge></div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4 pt-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-5">
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Key Competitors</span><p className="text-sm font-semibold text-slate-900">{account.key_competitors || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Incumbency Strength</span><p className="text-sm font-semibold text-slate-900">{account.incumbency_strength || '-'}</p></div>
                          <div className="space-y-1"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">White Spaces We Own</span><p className="text-sm font-semibold text-slate-900">{account.white_spaces_we_own || '-'}</p></div>
                          <div className="space-y-1 md:col-span-3"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Our Positioning</span><p className="text-sm leading-relaxed font-semibold text-slate-900">{account.our_positioning_vs_competition || '-'}</p></div>
                          <div className="space-y-1 md:col-span-3"><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Areas Competition Stronger</span><p className="text-sm font-semibold text-slate-900">{account.areas_competition_stronger || '-'}</p></div>
                        </div>
                      </div>
                          ...
                          {!stakeholders || stakeholders.length === 0 ? (
                            ...
                            <div className="border rounded-md min-w-[800px]">
                              <Table>
                                ...
                                <TableBody>
                                  {paginatedStakeholders.map((person: StrategicStakeholderProfile, idx: number) => (
            ...
            {/* Projects Cards — only shown in Account Details tab */}
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-3">
                  {filteredProjects.length > 10 && search === "" && (
                    <Button variant="outline" onClick={() => setShowAllProjects(!showAllProjects)} className="h-10 px-4 font-semibold border-slate-200 hover:border-blue-300 hover:bg-white text-blue-600 transition-all whitespace-nowrap shadow-sm">
                      {showAllProjects ? "Show Top 10" : `View All Projects (${filteredProjects.length})`}
                    </Button>
                  )}
                  <div className="relative">
                    <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
                    <Input placeholder="Search projects..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 w-[250px] shadow-sm bg-white" />
                  </div>
                </div>
              </div>
                ...
                {displayedProjects.map((project: FinanceProject) => (
                  <Card key={project.id} className="group hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer border-blue-100 hover:border-blue-300 border-t-4 border-t-blue-600 bg-gradient-to-br from-white to-blue-50/30" onClick={() => navigate(`/financials/${data.id}/projects/${project.id}`, { state: { backUrl: `/financials/${data.id}`, accountBackUrl: backUrl } })}>
                    <CardHeader className="p-4 pb-3 border-b border-blue-100/50 bg-gradient-to-r from-blue-50/50 to-transparent">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <div className="p-2 bg-blue-100/50 rounded-lg shrink-0 text-blue-600"><Briefcase className="w-5 h-5" /></div>
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                ...
                                <AlertDialogFooter>
                                  ...
                                  <AlertDialogAction className="bg-red-500 hover:bg-red-600" onClick={async (e) => { e.stopPropagation(); try { await deleteFinanceProject(project.id); const responseData = await getFinanceAccountById(data.id); setData(responseData); } catch (err) { console.error('Failed to delete project', err); } }}>Delete</AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </DropdownMenuContent>
                    ...
                    <CardContent className="p-4 grid grid-cols-2 gap-4">
                      <div className="space-y-1"><span className="text-xs uppercase font-bold text-slate-400 tracking-wider">Current Rev</span><p className="text-sm font-bold text-slate-900 truncate">{formatCurrency(project.total_revenue)}</p></div>
                      <div className="space-y-1"><span className="text-xs uppercase font-bold text-slate-400 tracking-wider">YTD Rev</span><p className="text-sm font-bold text-blue-600 truncate">{formatCurrency(project.ytd_revenue)}</p></div>
                      <div className="space-y-1"><span className="text-xs uppercase font-bold text-slate-400 tracking-wider">AI Rev</span><p className="text-sm font-bold text-emerald-600 truncate">{formatCurrency((project.ai_revenue || 0) + (project.ai_assisted_revenue || 0) || project.total_ai_revenue)}</p></div>
                      <div className="space-y-1"><span className="text-xs uppercase font-bold text-slate-400 tracking-wider">AI Pen.</span><p className="text-sm font-bold text-slate-900 truncate">{((project.ai_penetration || project.ai_penetration_pct || (project.total_revenue > 0 ? (((project.ai_revenue || 0) + (project.ai_assisted_revenue || 0)) / project.total_revenue) * 100 : 0)) || 0).toFixed(1)}%</p></div>
                      <div className="space-y-1 col-span-2"><span className="text-xs uppercase font-bold text-slate-400 tracking-wider">Type</span><p className="text-sm font-bold text-slate-700 truncate">{project.project_type || 'T&M'}</p></div>
                    </CardContent>
                  </Card>
                ))}
                {filteredProjects.length === 0 && (
                  <div className="col-span-full py-12 text-center text-gray-500 bg-white rounded-lg border border-gray-100 shadow-sm">No projects found.</div>
                )}
              </div>
            </div>
          </TabsContent>
